...
> Word 模板 + Excel 数据，一键生成批量文档（单镜像版）
...
2. 添加替换规则（模板关键字 -> Excel 列名，或从模板库加载）
3. 设置起始行、结束行、文件名列
4. 点击开始替换
5. 下载 ZIP 或合并文档

> ℹ️ 执行时使用“当前会话规则”（页面当前规则集合），不会强制套用数据库全局规则。

---

## 🧪 示例文件（快速体验）

为方便首次上手，项目内置了两组示例文件（均约 15 条数据）：

- 单页模板示例：
  - `frontend/public/examples/single_page_template.docx`
  - `frontend/public/examples/single_page_data.xlsx`
- 多页模板示例：
  - `frontend/public/examples/multi_page_template.docx`
  - `frontend/public/examples/multi_page_data.xlsx`

页面上传区可直接下载这些示例文件进行体验。  
...
services:
  wordreplace:
    image: ghcr.io/marod1m/wordreplace:v2.0.5
    container_name: wordreplace
    restart: unless-stopped
    ports:
      - "12344:8000"
...
1. 修改 compose 中镜像标签（例如 `v2.0.5`，版本号以仓库 `VERSION` 文件为准）
2. 执行：
...
项目使用根目录 `VERSION` 作为单一版本源，前后端显示与镜像构建均读取该值。  
如需调整版本号：

1. 修改 `VERSION`
2. 运行 `python3 scripts/sync_version.py`（同步 README/快速开始文案中的示例版本）
